...
__device__ vec3_half color(const ray &r, hitable **world,
                           curandState *local_rand_state) {
  ray cur_ray = r;
  vec3_half cur_attenuation = vec3_half(1.0, 1.0, 1.0);
  for (int i = 0; i < 50; i++) {
    hit_record rec;
    if ((*world)->hit(cur_ray, 0.001f, FLT_MAX, rec)) {
      ...
      if (rec.mat_ptr->scatter(cur_ray, rec, attenuation, scattered,
        ...
        return vec3_half(0.0f, 0.0f, 0.0f);
      }
    } else {
      vec3 unit_direction = unit_vector(cur_ray.direction());
      __half t = (__half)0.5f * ((__half)unit_direction.y() + (__half)1.0f);
      vec3_half c = ((__half)1.0f - t) * vec3_half(1.0f, 1.0f, 1.0f) +
                    t * vec3_half(0.5f, 0.7f, 1.0f);
      return cur_attenuation * c;
    }
  }
  return vec3_half(0.0, 0.0, 0.0); // exceeded recursion
}
...
__global__ void render(vec3_half *fb, int max_x, int max_y, int ns,
...
__global__ void create_world(hitable **d_list, hitable **d_world,
  ...
  if (threadIdx.x == 0 && blockIdx.x == 0) {
    ...
    d_list[0] = new sphere(vec3(0, -1000.0, -1), 1000,
                           new lambertian(vec3_half(0.5, 0.5, 0.5)));
    int i = 1;
    for (int a = -11; a < 11; a++) {
      for (int b = -11; b < 11; b++) {
        ...
        if (choose_mat < 0.8f) {
          d_list[i++] = new sphere(
              center, 0.2,
              new lambertian(vec3_half(RND * RND, RND * RND, RND * RND)));
        } else if (choose_mat < 0.95f) {
          d_list[i++] = new sphere(
              center, 0.2,
              new metal(vec3_half(0.5f * (1.0f + RND), 0.5f * (1.0f + RND),
                                  0.5f * (1.0f + RND)),
                        0.5f * RND));
        } else {
    ...
    d_list[i++] = new sphere(vec3(-4, 1, 0), 1.0,
                             new lambertian(vec3_half(0.4, 0.2, 0.1)));
    d_list[i++] = new sphere(vec3(4, 1, 0), 1.0,
                             new metal(vec3_half(0.7, 0.6, 0.5), 0.0));
...
int main() {
  ...
  size_t fb_size = num_pixels * sizeof(vec3_half);
