...
public:
  __device__ virtual bool scatter(const ray &r_in, const hit_record &rec,
                                  vec3_half &attenuation, ray &scattered,
                                  curandState *local_rand_state) const = 0;
};

class lambertian : public material {
public:
  __device__ lambertian(const vec3_half &a) : albedo(a) {}
  __device__ virtual bool scatter(const ray &r_in, const hit_record &rec,
                                  vec3_half &attenuation, ray &scattered,
...
public:
  __device__ metal(const vec3_half &a, float f) : albedo(a) {
    if (f < 1)
      fuzz = f;
    else
      fuzz = 1;
...
public:
  ...
  __device__ virtual bool scatter(const ray &r_in, const hit_record &rec,
    ...
    float ni_over_nt;
    attenuation = vec3_half(1.0, 1.0, 1.0);
    vec3 refracted;
    float reflect_prob;
    float cosine;
