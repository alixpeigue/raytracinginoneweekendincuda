...
__device__ DataType schlick(DataType cosine, DataType ref_idx) {
    DataType r0 = (1.0f-ref_idx) / (1.0f+ref_idx);
    r0 = r0*r0;
...
__device__ bool refract(const vec3& v, const vec3& n, DataType ni_over_nt, vec3& refracted) {
    ...
    DataType discriminant = 1.0f - ni_over_nt*ni_over_nt*(1-dt*dt);
    if (discriminant > 0) {
...
class metal : public material {
    public:
        __device__ metal(const vec3& a, DataType f) : albedo(a) { if (f < 1) fuzz = f; else fuzz = 1; }
        __device__ virtual bool scatter(const ray& r_in, const hit_record& rec, vec3& attenuation, ray& scattered, curandState *local_rand_state) const  {
